...
p18<-ggplot(ort_hane2, aes(x=ilce, y=ortalama_hanehalki_buyuklugu))+
...
p21<-ggplot(belediye_nufus2, aes(x=belediye, y=nufus_2019))+
  geom_bar(stat="identity")+
  theme(axis.text.x=element_text(angle=45, hjust=1))+
  xlab("Belediye Adı")+
  ylab("Ortalama Hanehalkı Büyüklüğü")+
...
Similarly, we can visualize the dataset titled "2019 Yılı Belediye Nüfusları". We can see from the graph that 2 municipalities, Esenyurt and Küçükçekmece, are over 750.000. We can also see from the graph that the first one-third of the municipalities are over 500.000. From this, we can identify the most populous municipalities that would need the most help in an earthquake disaster.
...
"İstanbul Çevresinde Gerçekleşen Depremlere ait Veriler ile Tahmin":
